from framework import file_txt, dsp_utils, data_types, parameter_estimation
import numpy as np
import matplotlib
...
for load in loads:
    data = file_txt.read(directory, fm=fm, n_periods=500, normalize_by=np.max) #organize the output in a SimuData structure

    #Sideband detection
    t_init = time.time()
    peaks = dsp_utils.fft_significant_peaks(data, harm_comps, method='distance', mag_threshold=-80, max_peaks=1, min_peak_dist=3.1) #run the peak detection routine
