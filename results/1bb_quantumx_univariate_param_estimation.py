...
t_init = time.time()
opt_height = parameter_estimation.univariate_gs_estimator(data, harm_comps, [0,3], 1e-3, plot=True)
...
t_init2 = time.time()
opt_height2 = parameter_estimation.univariate_gss_estimator(data, harm_comps, [0,3], tol=1e-3)
print(f'[param_estimation] GSS yielded opt. height diff. value in {time.time()-t_init2} s!')
print(f'[param_estimation] opt. height diff. = {opt_height2.opt_value}')
