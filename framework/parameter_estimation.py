...
def univariate_gs_estimator(data, harm_comps, bounds, step, mag_threshold=-80, k_real=14, plot=True):
    ...
    for thetas in range(len(candidate_values)):
        peaks = dsp_utils.fft_significant_peaks(data, harm_comps, method='distance', mag_threshold=mag_threshold, h_threshold=candidate_values[thetas]) #run the peak detection routine
        ...
        for harm_peak in peaks:
            harm_idx = np.argmin(np.abs(harm_peak[:,0]-v[harm_counter]*data.fm)) #the harmonic peak
        ...
        cost_values[thetas] = sqr_error + (k_real-k_hat)**2 #sum of all squared errors + the squared error between number of peaks and expected number
...
def multivariate_gs_estimator(file, data_type, load, harm_comps, p1_bounds, p2_bounds, p1_step, p2_step, mag_threshold=-80, k_real=14, normalize_by=np.max, plot=True):
    ...
    for p1_theta in range(len(p1_candidates)):
        ...
        for p2_theta in range(len(p2_candidates)):
            peaks = dsp_utils.fft_significant_peaks(data, harm_comps, method='distance', mag_threshold=mag_threshold, h_threshold=p2_candidates[p2_theta]) #run the peak detection routine
            sqr_error = 0 #variable to monitor the squared error between frequencies and number of detected peaks
            k_hat = 0 #variable to monitor the number of detected peaks per harmonic
            v = np.array([1, 5])[np.newaxis, :].T #harmonic components
            k = np.arange(-3, 4, 1)[np.newaxis, :] #fault orders
            analytical_signature = (v + 2 * k * data.slip) * data.fm #expected fault signatures location
            ...
            cost_values[p2_theta, p1_theta] = sqr_error + (k_real-k_hat)**2  #sum of all squared errors + the squared error between number of peaks and expected number
