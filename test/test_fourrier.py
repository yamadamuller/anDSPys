...
fft_data = dsp_utils.compute_FFT(data.i_motor) #compute the FFT of the current (shifted + normalized)
...
dft_data = np.abs(dft_data_r+dft_data_i)/np.max(dft_data_r+dft_data_i) #normalized magnitude of the DFT
...
plt.figure(1)
plt.subplot(3,1,1)
plt.plot(data.i_time_grid[freq_mask], data.i_motor[freq_mask])
plt.xlabel('t (s)')
